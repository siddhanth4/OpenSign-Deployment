...
//               scale
//             );
//             // Call cloud function only when pdfBytes is a valid base64 string (not undefined or error object)
//             if (pdfBytes && !pdfBytes.error) {
...
function SignYourSelf() {
  ...
  const signPdfFun = async (base64Url, documentId) => {
    ...
    // Guard: if no signature widget found at all, abort gracefully
    if (!getSignature) {
      setIsUiLoading(false);
      setIsAlert({
        header: t("error"),
    ...
    if (isUrl) {
      try {
        ...
        console.log("error fetching signature image", e);
        setIsUiLoading(false);
        alert(t("something-went-wrong-mssg"));
        return;
      }
